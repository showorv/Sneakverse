...
export default function CollectionSec() {
  ...
  useEffect(()=>{
    const fetchCollection = async ()=>{
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/products/stock`)
  ...
  return (
    <div className="container mx-auto p-4">
      {collection.length===0?( <div className="text-center text-xl text-gray-500">No products available</div>):(
      <Swiper
        slidesPerView={2}
        spaceBetween={15}

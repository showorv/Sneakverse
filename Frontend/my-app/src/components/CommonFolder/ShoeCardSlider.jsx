        ...
        {shoes.map((shoe) => (
          <SwiperSlide key={shoe.id}>
            <div className="bg-white rounded-xl shadow-lg p-4">
              <img src={shoe.image} alt={shoe.name} className="w-full h-64 object-cover rounded-lg mb-4" />
              <h2 className="text-xl font-semibold text-gray-800">{shoe.name}</h2>
              <p className="text-gray-500 text-sm mb-2">{shoe.description}</p>
              <p className="text-blue-600 text-lg font-bold">${shoe.price}</p>
